...
public class ReadWriteBlog {
  // Adjust this value to try the different approaches mentioned in the blog.
  // 1: no r/w splitting
  ...
  static final boolean DELETE_TABLES_ON_STARTUP = false;
  static final String WRITER_CLUSTER =
      "jdbc:aws-wrapper:postgresql://test-db.cluster-XYZ.us-east-2.rds.amazonaws.com/readWriteBlog";
  static final String USER = "username";
  static final String PASSWORD = "password";
  static final Semaphore sem = new Semaphore(MAX_SIMULTANEOUS);
  static final Logger LOGGER = Logger.getLogger(ReadWriteBlog.class.getName());
  ...
  public static void main(String[] args) throws SQLException {
    ...
    try {
      for (int i = 0; i < NUM_THREADS; i++) {
        if (APPROACH_ID == 1) {
          executorService.submit(new Approach1Thread(i));
        } else {
          executorService.submit(new RWThread(i, props));  // RWThread should be used for approach 2 and approach 3.
        }
      ...
      try (Connection conn = DriverManager.getConnection(WRITER_CLUSTER, props)) {
        Statement stmt = conn.createStatement();
        for (int i = 0; i < NUM_THREADS; i++) {
          stmt.execute(String.format("drop table if exists rw_blog_%d", i));
        }
      }
    }
  }
  ...
  private static void executeWrites(Connection conn, int tableNum) throws SQLException {
    long start = System.nanoTime();
    String table = String.format("rw_blog_%d", tableNum);
    Statement stmt = conn.createStatement();
    stmt.execute(String.format("create table %s(some_num int not null)", table));

    String insertSQL = String.format("insert into %s values (1)", table);
  ...
  static class Approach1Thread implements Callable<Void> {
    private final int id;

    Approach1Thread(int id) {
      this.id = id;
    }
  ...
  static class RWThread implements Callable<Void> {
    private final int id;
    private final Properties props;

    RWThread(int id, Properties props) {
      this.id = id;
      this.props = props;
    }
